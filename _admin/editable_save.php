...
if(!empty($_POST['id']) && !empty($_POST['type']) && !empty($_POST['area']) && is_numeric($_POST['id'])) {
	...
	if($_POST['type']=='ht') {
		// this is a HOT TOPIC
		$sql_update = "UPDATE author_topics SET ".$db_field." = '".mysql_real_escape_string($_POST['value'])."',
						Updated = '".mysql_real_escape_string($now_timestamp)."',
						UpdatedID = '".mysql_real_escape_string($user_id)."'
	...
	elseif($_POST['type']=='e') {
		...
		$sql_update = "UPDATE news_details SET ".$db_field." = '".mysql_real_escape_string($_POST['value'])."',
						Updated = '".mysql_real_escape_string($now_timestamp)."',
						UpdatedID = '".mysql_real_escape_string($user_id)."'
						WHERE ID = '".mysql_real_escape_string($edit_id)."'";
		mysqli_query($connect_admin, $sql_update);
